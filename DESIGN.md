...
* **On a `td`**: anything that does. The "has comments" gutter tick is `tr.commentable-element.has-comments-highlight > td:first-child::before`, and `updateCommentIndicators()` appends the comment indicator to the row's last cell rather than to the row. It keeps its absolute positioning into the right gutter, so its on-screen place is unchanged.
...
* **`data-anchor` goes on the first line of a range only**, with the remaining lines marked
  ...
  `querySelector('[data-anchor="…"]')` — the connector line, the comment indicator, panel ordering,
